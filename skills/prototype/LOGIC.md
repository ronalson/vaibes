...
Put the logic that answers the question in a single `<script>` block as a small, pure module. Keep the page around it throwaway. If the idea is later reused, rewrite and verify it under the destination project's requirements.
...
Pick whichever shape best fits the question being asked, *not* whichever is easiest to wire to a page. Keep it pure: no DOM, no `document`, no button handlers reaching inside it. The page calls into it; nothing flows the other direction. The runnable demo supports a decision; if its logic is later used in production, rewrite and verify it under the destination project's requirements.
...
State the observed result and the decision it supports, then provide the runnable HTML file. Do not promote the module, archive the prototype, commit it, or update an issue unless the user asks.
...
- **Don't blur the logic and the page together.** If the pure module references the DOM, `document`, or button handlers, it cannot be evaluated separately. Keep the page as a thin shell over a pure module.
...
- **Don't ship the HTML shell or use its logic directly in production.** The page is optimised for being clicked through by hand, and any production reuse must be rewritten and verified for the destination project.
